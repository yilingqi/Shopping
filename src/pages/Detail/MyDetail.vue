    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuImageList" /><!--需要父组件传过去的图片-->
          <!-- 小图列表 -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addShopCar">加入购物车</a><!--在这里路由跳转的时候还要发送一个请求，把购买信息传给服务器-->
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  //当挂载的时候（在search中点击图片跳转到这里），我们需要获取仓库中的数据，所以，在挂载的时候派发action
  mounted() {
    this.$store.dispatch("Detail/getGoodInfo", this.$route.params.skuid); //因为这里要求传参，所以要把路由中的id也一起带过去
  },
  components: {
    ImageList,
    Zoom,
  ...
  computed: {
    ...
    //异步操作会有渲染问题，它加载的时候，父亲数据没有传回来，需要先给一个[]
  },
  methods:{
    changeActive( spuSaleAttrValue, spuSaleAttrValueList){
      //遍历,先把所有元素身上的isChecked给置0
    ...
    async addShopCar(){
      try{await this.$store.dispatch('Detail/addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
          //进行路由跳转
          //在这里进行路由跳转的时候，去要带过去参数，但是不需要用路由传参，因为对象里面数据很多
          //直接本地存储
      ...
      //1.发请求，将产品加入数据库（通知服务器），2.成功，路由跳转（带参）
     //当前的这个函数，返回的是一个promise
    }
  }
};
</script>
